...
class NavigationViewSet(MyViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        for one in base_ret:
            ...
            one["lessons"] = LessonSerializer(
                lesson_list.filter(Q(type__code=1) & Q(category__code__in=one["code"].split(",")))[:4], many=True).data
...
class LabelViewSet(MyViewSet):
    ...
    def follow(self, request, pk=None):
        ...
        add_list = []
        for one in list:
            one_follow = LabelFollow(userid=request.user.id, title=one["title"],
                                     labelid=Label.objects.get(title=one["title"]).id)
...
class RecommendView(APIView):
    def get(self, request):
        ...
        return JsonResponse(json.load(open(data_json, 'r', encoding='gb18030', errors='ignore')))
...
class HomeLessonView(APIView):
    ...
    def post(self, *args, **kwargs):
        ...
        easy_list = Lesson.objects.filter(hard__code=0)[:10]
        easy_lista = Lesson.objects.raw('select * from app_api_lesson where hard_id=145')[:10]
        return JsonResponse({'ok': LessonSerializer(easy_list, many=True).data,
                             'raw': LessonSerializer(easy_lista, many=True).data})
...
async def yibu(request):
    # loop = asyncio.get_event_loop()
    # loop.create_task(slep())
    d = await slep()
    print(d)
